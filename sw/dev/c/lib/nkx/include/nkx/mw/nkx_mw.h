...
uint8_t onPad(uint8_t button);
uint8_t offPad(uint8_t button);
uint8_t pressedPad(uint8_t button);
uint8_t releasedPad(uint8_t button);

/* Dma */
void setDma(uint8_t ch, uint32_t src, uint32_t dst, uint32_t size);
void startDma(uint8_t ch);
