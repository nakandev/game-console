...
void setTilemap(uint8_t mapNo, uint32_t offset, const uint16_t* mapData, uint32_t size)
{
  HwTileRam& tileram = *(HwTileRam*)HWREG_TILERAM_BASEADDR;
  for (int i=0; i<size; i++) {
    tileram.tilemap[mapNo].tileIdx[i].data = mapData[i];
  }
}
...
void setTilemap2D(
  ...
  for (int y=0; y<dataRectH; y++) {
    for (int x=0; x<dataRectW; x++) {
      ...
      tileram.tilemap[mapNo].tileIdx[tileIdx].data = mapData[dataIdx];
    }
  }
}
...
void setTilemap2DLambda(
  ...
  for (int y=0; y<dataRectH; y++) {
    for (int x=0; x<dataRectW; x++) {
      ...
      tileram.tilemap[mapNo].tileIdx[tileIdx].data = mapData(dataIdxX, dataIdxY);
    }
  }
}
